...
### #1. [MVP] アプリ雛形（Next.js + TS + Tailwind）

- 目的: 最小のWebアプリ骨格を作り、以降の実装の土台を揃える
- AC:
  - [x] ルーティング（/ /login /dashboard /deadline/new /billing /billing/success /billing/cancel）が作れる状態
  - [x] ローカル起動手順がREADMEに1段落で書かれている
- 優先度: P0
- 依存関係: なし
- 推定: M

### #2. [MVP] Prisma + Postgres スキーマ作成（ADR 0002 準拠）
...
### #3. [MVP] メールマジックリンク認証（ログイン/サインアップ共通）
...
### #4. [MVP] Deadline Item 作成API（バリデーション+Free枠制限）
...
### #5. [MVP] Deadline Item 一覧API（締切近い順）

- 目的: ダッシュボード表示に必要な一覧取得を提供する
- AC:
  - [x] `GET /api/deadlines` がログインユーザーのアイテムのみ返す
  - [x] `deadline_at` 昇順（近い順）で返す
- 優先度: P0
- 依存関係: #2, #3
- 推定: S

### #6. [MVP] Deadline Item 更新/削除API（ステータス更新含む）
...
### #7. [MVP] LP（/）実装（価値訴求/価格/CTA）

- 目的: SNS→LP→ログインの導線を成立させる
- AC:
  - [x] 価値提案（締切ミス防止）・価格（980円/月）・Free/Pro差分が載っている
  - [x] CTAが /login へ遷移する
- 優先度: P1
- 依存関係: #1
- 推定: S

### #8. [MVP] ダッシュボードUI（締切近い順・空状態・ステータス即更新）

- 目的: コア体験（近い順確認→即更新）を実現する
- AC:
  - [x] 締切が近い順に表示される
  - [x] ステータス変更が1操作で反映される（API連携）
  - [x] 空状態（0件）で「2件登録」を促す導線がある
- 優先度: P0
- 依存関係: #5, #6
- 推定: M

### #9. [MVP] 締切作成フォーム（/deadline/new）

- 目的: 2件登録（Activation）を最短で達成できる入力体験を作る
- AC:
  - [x] 必須項目（企業名/種別/締切日時）が入力でき、作成後に /dashboard へ戻る
  - [x] 入力エラー時に理由が表示される
- 優先度: P0
- 依存関係: #4
- 推定: M

### #10. [MVP] Stripe Checkout 作成API（/billing からUpgrade）
...
### #11. [MVP] Stripe Webhook（署名検証→subscriptions upsert）
...
### #13. [MVP] /billing/success と /billing/cancel の着地ページ
...
### #14. [MVP] Stripe Customer Portal API（解約/支払い方法変更）
...
### #15. [MVP] 通知Cron（対象抽出→送信→notification_deliveries更新）
...
### #16. [MVP] メール送信基盤（プロバイダ設定 + 送信ユーティリティ）

- 目的: 認証/通知で使うメール送信の土台を先に固める
- AC:
  - [x] dev/prodでメール送信ができる（環境変数で切替）
  - [x] 送信失敗時にエラーを捕捉でき、運用で追える形（ログ等）になっている
- 優先度: P0
- 依存関係: #1
- 推定: M

### #17. [MVP] 計測（signup/activation/dashboard_viewed/purchase）
...
### #18. [MVP] 環境変数・運用の最低限（.env.example/Secrets/Runbook草案）

- 目的: 本番運用で詰まりやすい設定/手順を先に可視化し、事故を減らす
- AC:
  - [x] `.env.example` に必須/任意が整理されている（Stripe/Webhook/Cron/メール/DB/計測）
  - [x] Cron/Webhookのシークレット運用（どこに設定するか）が短く書かれている
  - [x] 失敗時の一次切り分け（ログの場所/再実行可否/ロールバック方針）が1ページにまとまっている
- 優先度: P1
- 依存関係: #10, #11, #15
- 推定: S
